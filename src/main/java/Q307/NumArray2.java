...
class NumArray2 {
  ...
  private interface Merger<E> {
    /**
     * 定义线段树中如何由一个结点的左右孩子结点融合出该节点的值。 例如： - 是左右结点中的最小值 - 是左右结点值之和 - ....
     *
     * @param left
     * @param right
     * @return
  ...
  private class SegmentTree<E> {
    ...
    /**
     * 构建线段树 base case: 区间只有一个元素，即 l == r recursion: 对左半个区间[l, mid]建立左子树 对右半个区间[mid + 1, r]建立右子树 树 =
     * 根节点 + 左子树 + 右子树
     *
    ...
    private E query(int treeIndex, int l, int r, int queryL, int queryR) {
      ...
      if (queryR <= mid) { // 区间全在左半部分
        ...
        return merger.merge(
            query(leftChild, l, mid, queryL, mid), query(rightChild, mid + 1, r, mid + 1, queryR));
      }
    }
